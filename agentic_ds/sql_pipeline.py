...
# The LLM is asked for SELECT-only SQL, but a prompt is not a security
# boundary — an untrusted/hallucinating model could still emit e.g.
# ATTACH DATABASE (opens an arbitrary file as a second DB) or a
# schema/data-modifying statement. sqlite3's DBAPI already refuses to
# execute more than one ;-separated statement per call, so the remaining
# risk is a single dangerous statement; block that explicitly. This is a
# sandbox around what one query can do, not a full untrusted-code sandbox
# (see PLAN.md's Phase 3 note on run_python_code/run_sql_query).
_SQL_BLOCKLIST = re.compile(
    r"\b(attach|detach|pragma|vacuum|drop|delete|insert|update|alter|create|replace|reindex)\b",
    re.IGNORECASE,
)


def _reject_unsafe_sql(sql: str) -> str | None:
    """Return an error message if `sql` isn't a plain read-only SELECT, else None."""
    if not re.match(r"^\s*(select|with)\b", sql, re.IGNORECASE):
        return "Only SELECT statements are allowed."
    if _SQL_BLOCKLIST.search(sql):
        return "Query contains a disallowed keyword (schema/data-modifying or file-attaching statements are blocked)."
    return None
...
def _run_sql(sql: str, df: pd.DataFrame) -> tuple[pd.DataFrame | None, str | None]:
    ...
    unsafe_reason = _reject_unsafe_sql(sql)
    if unsafe_reason:
